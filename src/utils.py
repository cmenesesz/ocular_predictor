...
def generate_square_subsequent_mask(dim1: int, dim2: int, device: str='cpu') -> Tensor:
    """
    ...
    Args:
        ...
        device: Device of tensors. Default as 'cpu'.
...
def get_indices_entire_sequence_old(data: pd.DataFrame, window_size: int, step_size: int) -> list:
        """
...
def get_indices_entire_sequence(data: torch.tensor, window_size: int, step_size: int) -> list:
        """
        Produce all the start and end index positions that is needed to produce
        the sub-sequences. 
...
def split_dataset(dataset, validation_portion, test_portion=0, seed=0):
    assert validation_portion+test_portion<1
    assert 0<validation_portion<1
    assert 0<=test_portion<1

    n_val = int(len(dataset) * validation_portion)
    n_test = int(len(dataset) * test_portion)
